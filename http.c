...
static bool set_request_method(CURL *handle, http_method method, const discord_http_request_options *opts){
    if (!handle){
        log_write(
            logger,
            LOG_ERROR,
    ...
    case HTTP_POST:
        ...
        break;
    case HTTP_PUT:
        err = curl_easy_setopt(handle, CURLOPT_PUT, 1);
...
discord_http *http_init(const char *token, const discord_http_options *opts){
    if (!opts){
        /* unused for now */
    }
...
discord_http_response *http_request(discord_http *http, http_method method, const char *path, const discord_http_request_options *opts){
    if (!http){
        log_write(
            logger,
            LOG_ERROR,
    ...
    if (is_rate_limited(http, bucket)){
        log_write(
            logger,
            LOG_WARNING,
            "[%s] http_request() - refusing request, rate limited (bucket: %s)\n",
            __FILE__,
...
discord_http_response *http_get_gateway(discord_http *http){
    return http_request(http, HTTP_GET, "/gateway", NULL);
}

discord_http_response *http_get_bot_gateway(discord_http *http){
...
discord_http_response *http_get_user(discord_http *http, snowflake userid){
    ...
    discord_http_response *response = http_request(http, HTTP_GET, path, NULL);
...
discord_http_response *http_modify_current_user(discord_http *http, const char *username, const char *avatar){
...
discord_http_response *http_get_current_user_guilds(discord_http *http, snowflake before, snowflake after, int limit){
    json_object *data = json_object_new_object();
...
discord_http_response *http_create_dm(discord_http *http, snowflake recipientid){
    json_object *data = json_object_new_object();
...
/*
discord_http_response *http_create_group_dm(discord_http *http, const json_object *data){

}
*/

discord_http_response *http_get_user_connections(discord_http *http){
    return http_request(http, HTTP_GET, "/users/@me/connections", NULL);
}
...
void http_response_free(discord_http_response *response){
    if (!response){
        log_write(
            logger,
            LOG_DEBUG,
...
void http_free(discord_http *http){
    if (!http){
        log_write(
            logger,
            LOG_DEBUG,
